...
export function toGrpcClientError(error: unknown): GrpcClientError {
  ...
  if (error instanceof ConnectError) {
    return new GrpcClientError(error.rawMessage || error.message, error.code, headersToRecord(error.metadata));
  }
...
export function isGrpcCode(error: unknown, code: Code): boolean {
  if (error instanceof ConnectError) {
