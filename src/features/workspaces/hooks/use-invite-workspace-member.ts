...
export function useInviteWorkspaceMember() {
  ...
  const inviteWorkspaceMember = useCallback(async (input: InviteWorkspaceMemberInput) => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      const message = e instanceof Error ? e.message : 'Failed to invite workspace member';
      setError(message);
      throw e;
    } finally {
      setLoading(false);
