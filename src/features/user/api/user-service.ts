...
import { ChangePasswordRequestSchema, GetMeRequestSchema, UpdateProfileRequestSchema } from '@/gen/chatapp/user/v1/user_service_pb';
import { toGrpcClientError } from '@/lib/grpc/error';

import type { ChangePasswordInput, UpdateProfileInput, UserModel } from '../model/user-types';
import { mapGetMeResponse, mapUpdateProfileResponse } from '../utils/user-mapper';
...
export const userService = {
  async getMe(): Promise<UserModel | null> {
    try {
      const response = await userClient.getMe(create(GetMeRequestSchema, {}));
