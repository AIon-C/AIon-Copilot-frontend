...
var CoreToolBuilder = class extends chunkRO47SMI7_cjs.MastraBase {
  ...
  constructor(input) {
    ...
    if (!chunkE7PW4ZAB_cjs.isVercelTool(this.originalTool) && (input.autoResumeSuspendedTools || this.originalTool.id?.startsWith("agent-") || this.originalTool.id?.startsWith("workflow-"))) {
      ...
      if (chunkRLPRSIR2_cjs.isZodObject(schema)) {
        ...
        const jsonSchema2 = chunkVWJ7H3MJ_cjs.standardSchemaToJSONSchema(schema, { io: "input" });
        if (jsonSchema2 && typeof jsonSchema2 === "object" && jsonSchema2.type === "object") {
          ...
          this.originalTool.inputSchema = chunkVWJ7H3MJ_cjs.toStandardSchema(jsonSchema2);
        }
      }
    }
  }
  ...
  buildProviderTool(tool) {
    if ("type" in tool && (tool.type === "provider-defined" || tool.type === "provider") && "id" in tool && typeof tool.id === "string" && tool.id.includes(".")) {
      ...
      if (parameters !== void 0 && parameters !== null) {
        ...
        } else if (chunkVWJ7H3MJ_cjs.isStandardSchemaWithJSON(parameters)) {
          const jsonSchema2 = chunkVWJ7H3MJ_cjs.standardSchemaToJSONSchema(parameters, { io: "output" });
      ...
      if (outputSchema !== void 0 && outputSchema !== null) {
        ...
        } else if (chunkVWJ7H3MJ_cjs.isStandardSchemaWithJSON(outputSchema)) {
          const jsonSchema2 = chunkVWJ7H3MJ_cjs.standardSchemaToJSONSchema(outputSchema);
  ...
  createExecute(tool, options, logType) {
    ...
    const execFunction = async (args, execOptions) => {
      ...
      const mcpMeta = !chunkE7PW4ZAB_cjs.isVercelTool(tool) && "mcpMetadata" in tool ? tool.mcpMetadata : void 0;
      const toolRequestContext = execOptions.requestContext ?? options.requestContext;
      const toolSpan = chunkW26XAFT3_cjs.getOrCreateSpan({
      ...
      try {
        ...
        if (chunkE7PW4ZAB_cjs.isVercelTool(tool)) {
          result = await chunkW26XAFT3_cjs.executeWithContext({
            span: toolSpan,
            fn: async () => tool?.execute?.(args, execOptions)
          });
        } else {
          const wrappedMastra = options.mastra ? chunkW26XAFT3_cjs.wrapMastra(options.mastra, { currentSpan: toolSpan }) : options.mastra;
          ...
          const baseContext = {
            ...
            ...chunkW26XAFT3_cjs.createObservabilityContext({ currentSpan: toolSpan }),
            abortSignal: execOptions.abortSignal,
            suspend: (args2, suspendOptions) => {
              suspendData = args2;
              const newSuspendOptions = {
                ...suspendOptions ?? {},
                resumeSchema: suspendOptions?.resumeSchema ?? (resumeSchema ? JSON.stringify(chunkVWJ7H3MJ_cjs.standardSchemaToJSONSchema(chunkVWJ7H3MJ_cjs.toStandardSchema(resumeSchema), { io: "input" })) : void 0)
              };
          ...
          result = await chunkW26XAFT3_cjs.executeWithContext({ span: toolSpan, fn: async () => tool?.execute?.(args, toolContext) });
        }
        if (suspendData) {
  ...
  build() {
    ...
    if (chunkVWJ7H3MJ_cjs.isStandardSchemaWithJSON(originalSchema)) {
      const inputJsonSchema = applicableLayer ? applicableLayer.toJSONSchema(originalSchema) : chunkVWJ7H3MJ_cjs.standardSchemaToJSONSchema(originalSchema, { io: "input" });
    ...
    let processedOutputSchema;
    if (outputSchema) {
      if (chunkVWJ7H3MJ_cjs.isStandardSchemaWithJSON(outputSchema)) {
        processedOutputSchema = chunkVWJ7H3MJ_cjs.standardSchemaToJSONSchema(outputSchema, { io: "output" });
      } else {
...
function resolveSerializedZodOutput(schema) {
  return Function("z", `"use strict";return (${schema});`)(v3.z);
}
...
function ensureToolProperties(tools) {
  const toolsWithProperties = Object.keys(tools).reduce((acc, key) => {
    ...
    if (tool) {
      if (typeof tool === "function" && !(tool instanceof chunkE7PW4ZAB_cjs.Tool) && !chunkE7PW4ZAB_cjs.isVercelTool(tool)) {
...
function convertVercelToolParameters(tool) {
  let schema = tool.parameters ?? v3.z.object({});
  if (typeof schema === "function") {
    schema = schema();
  }
...
//# sourceMappingURL=chunk-CYZJ2HT2.cjs.map
//# sourceMappingURL=chunk-CYZJ2HT2.cjs.map