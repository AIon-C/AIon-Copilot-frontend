...
var chunkMRNTJIUP_cjs = require('../chunk-MRNTJIUP.cjs');
...
var planCounter = 0;
var askUserTool = chunkE7PW4ZAB_cjs.createTool({
  id: "ask_user",
...
var submitPlanTool = chunkE7PW4ZAB_cjs.createTool({
  id: "submit_plan",
...
var taskWriteTool = chunkE7PW4ZAB_cjs.createTool({
  id: "task_write",
...
var taskCheckTool = chunkE7PW4ZAB_cjs.createTool({
  id: "task_check",
...
function createSubagentTool(opts) {
  ...
  return chunkE7PW4ZAB_cjs.createTool({
    id: "subagent",
...
- The task is self-contained and can be delegated`,
    ...
    execute: async ({ agentType, task, modelId }, context) => {
      ...
      const subagent = new chunkF5254W6E_cjs.Agent({
      ...
      const allWorkspaceToolNames = workspace ? new Set(Object.keys(chunkHACNMOMO_cjs.createWorkspaceTools(workspace))) : void 0;
...
var Harness = class {
  ...
  constructor(config) {
    ...
    this.stateSchema = config.stateSchema ? chunkVWJ7H3MJ_cjs.toStandardSchema(config.stateSchema) : void 0;
    this.state = {
    ...
    if (config.workspace instanceof chunkHACNMOMO_cjs.Workspace) {
  ...
  async init() {
    if (this.config.storage) {
      this.#internalMastra = new chunkMRNTJIUP_cjs.Mastra({ logger: false, storage: this.config.storage });
      await this.#internalMastra.getStorage().init();
    }
    if (this.config.workspace && !this.workspaceInitialized && !this.workspaceFn) {
      try {
        if (!this.workspace) {
          this.workspace = new chunkHACNMOMO_cjs.Workspace(this.config.workspace);
        }
  ...
  async getCurrentModelAuthStatus() {
    ...
    try {
      const { PROVIDER_REGISTRY } = await import('../provider-registry-73GVT5AT.cjs');
