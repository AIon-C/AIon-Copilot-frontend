...
var AIV5Adapter = class {
  ...
  static fromUIMessage(uiMsg) {
    ...
    const toolInvocationParts = parts.filter((p) => chunkFXGVRQSG_cjs.isToolUIPart(p));
    ...
    const v2Parts = parts.map((p) => {
      if (chunkFXGVRQSG_cjs.isToolUIPart(p)) {
        const toolName = getToolName(p);
...
var CacheKeyGenerator = class _CacheKeyGenerator {
  ...
  static fromAIV5Parts(parts) {
    ...
    for (const part of parts) {
      ...
      if (chunkFXGVRQSG_cjs.isToolUIPart(part) || part.type === "dynamic-tool") {
        key += part.toolCallId;
        key += part.state;
      }
      if (part.type === "reasoning") {
...
function sanitizeV5UIMessages(messages, filterIncompleteToolCalls = false) {
  const msgs = messages.map((m) => {
    ...
    const safeParts = m.parts.filter((p) => {
      ...
      if (!chunkFXGVRQSG_cjs.isToolUIPart(p)) return true;
    ...
    const sanitized = {
      ...
      parts: safeParts.map((part) => {
        ...
        if (chunkFXGVRQSG_cjs.isToolUIPart(part) && part.state === "output-available") {
          return {
            ...part,
...
function addStartStepPartsForAIV5(messages) {
  for (const message of messages) {
    ...
    for (const [index, part] of message.parts.entries()) {
      if (!chunkFXGVRQSG_cjs.isToolUIPart(part)) continue;
      const nextPart = message.parts.at(index + 1);
      if (nextPart && nextPart.type !== `step-start` && !chunkFXGVRQSG_cjs.isToolUIPart(nextPart)) {
...
function aiV4UIMessagesToAIV4CoreMessages(messages) {
  return chunkAXQA7AJE_cjs.convertToCoreMessages(sanitizeAIV4UIMessages(messages));
}
function aiV5UIMessagesToAIV5ModelMessages(messages, dbMessages, filterIncompleteToolCalls = false) {
  ...
  const result = chunkFXGVRQSG_cjs.convertToModelMessages(preprocessed);
...
var StepContentExtractor = class _StepContentExtractor {
  ...
  static convertPartsToContent(parts, stepId, stepContentFn) {
    ...
    const modelMessages = chunkFXGVRQSG_cjs.convertToModelMessages(sanitizeV5UIMessages(stepUiMessages));
...
//# sourceMappingURL=chunk-CLKMJINC.cjs.map
//# sourceMappingURL=chunk-CLKMJINC.cjs.map