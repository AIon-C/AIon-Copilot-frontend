import { DefaultVoice } from './chunk-73REJWMW.js';
import { PUBSUB_SYMBOL, STREAM_FORMAT_SYMBOL } from './chunk-2QXNHEDL.js';
import { createWorkspaceTools, createSkillTools, getTiktoken, BM25Index } from './chunk-RHU25VWO.js';
import { InMemoryStore } from './chunk-ZNIHEZDL.js';
import { MessageList, coreContentToString, DefaultGeneratedFile, DefaultGeneratedFileWithType, sanitizeToolName } from './chunk-ESN5IH6I.js';
import { parsePartialJson, isDeepEqualData, stepCountIs } from './chunk-KZEEMJ2X.js';
import { generateId, APICallError, tool, asSchema } from './chunk-QYBXBWWW.js';
import { executeHook } from './chunk-L54GIUCB.js';
import { resolveModelConfig, ModelRouterEmbeddingModel, ModelRouterLanguageModel } from './chunk-7DGI2TU4.js';
import { MastraLLMV1 } from './chunk-2QU52I7Q.js';
import { noopLogger } from './chunk-PSCMWPLC.js';
import { EventEmitterPubSub } from './chunk-X2HAI3OX.js';
import { removeUndefinedValues, isZodType, ensureToolProperties, makeCoreTool, createMastraProxy, deepMerge, generateEmptyFromSchema, selectFields, delay } from './chunk-FNVUZ3S2.js';
import { resolveObservabilityContext, createObservabilityContext, getOrCreateSpan, wrapMastra, executeWithContextSync } from './chunk-DYVLPDXH.js';
import { ToolStream } from './chunk-DD2VNRQM.js';
import { Tool, createTool, isMastraTool } from './chunk-7UM6FE6A.js';
import { toStandardSchema, standardSchemaToJSONSchema, isStandardSchemaWithJSON } from './chunk-6VAVASZ5.js';
...
import { __commonJS, __toESM } from './chunk-TM6AOUSD.js';
import z9, { z } from 'zod/v4';
...
import { wrapSchemaWithNullTransform, isZodType as isZodType$1, prepareJsonSchemaForOpenAIStrictMode } from '@mastra/schema-compat';
...
function createOutputHandler({ schema }) {
  const normalizedSchema = schema && !isStandardSchemaWithJSON(schema) ? toStandardSchema(schema) : schema;
...
var messageListSchema = z.custom().describe("MessageList instance for managing message sources");
...
var outputResultSchema = z.object({
...
function createStepFromAgent(params, agentOrToolOptions) {
  ...
  const outputSchema = toStandardSchema(
    options?.structuredOutput?.schema ?? z.object({ text: z.string() })
  );
...
function createStepFromProcessor(processor) {
  ...
  return {
    ...
    inputSchema: toStandardSchema(ProcessorStepInputSchema),
...
var Workflow = class extends MastraBase {
  ...
  constructor({
    ...
    this.stateSchema = stateSchema ? toStandardSchema(stateSchema) : void 0;
    this.requestContextSchema = requestContextSchema ? toStandardSchema(requestContextSchema) : void 0;
...
var languageModelUsageSchema = z9.object({
  inputTokens: z9.number().optional(),
  outputTokens: z9.number().optional(),
  totalTokens: z9.number().optional(),
  reasoningTokens: z9.number().optional(),
  cachedInputTokens: z9.number().optional()
});
var llmIterationStepResultSchema = z9.object({
  reason: z9.string(),
  warnings: z9.array(z9.any()),
  isContinued: z9.boolean(),
  logprobs: z9.any().optional(),
  totalUsage: languageModelUsageSchema.optional(),
  headers: z9.record(z9.string(), z9.string()).optional(),
  messageId: z9.string().optional(),
  request: z9.record(z9.string(), z9.any()).optional()
});
var llmIterationOutputSchema = z9.object({
  messageId: z9.string(),
  messages: z9.object({
    all: z9.array(z9.any()),
    // ModelMessage[] but too complex to validate at runtime
    user: z9.array(z9.any()),
    nonUser: z9.array(z9.any())
  }),
  output: z9.object({
    text: z9.string().optional(),
    reasoning: z9.array(z9.any()).optional(),
    reasoningText: z9.string().optional(),
    files: z9.array(z9.any()).optional(),
    // GeneratedFile[]
    toolCalls: z9.array(z9.any()).optional(),
    // TypedToolCall[]
    toolResults: z9.array(z9.any()).optional(),
    // TypedToolResult[]
    sources: z9.array(z9.any()).optional(),
    // LanguageModelV2Source[]
    staticToolCalls: z9.array(z9.any()).optional(),
    dynamicToolCalls: z9.array(z9.any()).optional(),
    staticToolResults: z9.array(z9.any()).optional(),
    dynamicToolResults: z9.array(z9.any()).optional(),
    usage: languageModelUsageSchema,
    steps: z9.array(z9.any())
    // StepResult[]
  }),
  metadata: z9.object({
    ...
    modelMetadata: z9.object({
      modelId: z9.string(),
      modelVersion: z9.string(),
      modelProvider: z9.string()
    }).optional(),
    timestamp: z9.date().optional(),
    providerMetadata: z9.record(z9.string(), z9.any()).optional(),
    headers: z9.record(z9.string(), z9.string()).optional(),
    request: z9.record(z9.string(), z9.any()).optional()
  }),
  stepResult: llmIterationStepResultSchema,
  processorRetryCount: z9.number().optional(),
  processorRetryFeedback: z9.string().optional(),
  isTaskCompleteCheckFailed: z9.boolean().optional()
  //true if the isTaskComplete check failed and LLM has to run again
});
var toolCallInputSchema = z9.object({
  toolCallId: z9.string(),
  toolName: z9.string(),
  args: z9.record(z9.string(), z9.any()),
  providerMetadata: z9.record(z9.string(), z9.any()).optional(),
  providerExecuted: z9.boolean().optional(),
  output: z9.any().optional()
});
var toolCallOutputSchema = toolCallInputSchema.extend({
  result: z9.any().optional(),
...
function createLLMMappingStep({ models, _internal, ...rest }, llmExecutionStep) {
  ...
  return createStep({
    ...
    inputSchema: z9.array(toolCallOutputSchema),
    outputSchema: llmIterationOutputSchema,
...
var PRIMITIVE_TYPES = z9.enum(["agent", "workflow", "none", "tool"]);
...
async function getRoutingAgent({
  ...
  const workflowList = Object.entries(workflowsToUse).map(([name, workflow]) => {
    return ` - **${name}**: ${workflow.description}, input schema: ${JSON.stringify(
      schemaToJsonSchema(workflow.inputSchema ?? z9.object({}))
    )}`;
  }).join("\n");
  ...
  const toolList = Object.entries({ ...toolsToUse, ...memoryTools, ...clientToolsToUse || {} }).map(([name, tool2]) => {
    const inputSchema = "inputSchema" in tool2 ? tool2.inputSchema ?? z9.object({}) : z9.object({});
...
async function createNetworkLoop({
  ...
  const routingStep = createStep({
    ...
    inputSchema: z9.object({
      task: z9.string(),
      primitiveId: z9.string(),
      primitiveType: PRIMITIVE_TYPES,
      result: z9.string().optional(),
      iteration: z9.number(),
      threadId: z9.string().optional(),
      threadResourceId: z9.string().optional(),
      isOneOff: z9.boolean(),
      verboseIntrospection: z9.boolean()
    }),
    outputSchema: z9.object({
      ...
      prompt: z9.string(),
      result: z9.string(),
      isComplete: z9.boolean().optional(),
      selectionReason: z9.string(),
      iteration: z9.number(),
      conversationContext: z9.array(z9.any()).optional()
    }),
    execute: async ({ inputData, getInitData, writer }) => {
      ...
      const options = {
        structuredOutput: {
          schema: z9.object({
            primitiveId: z9.string().describe("The id of the primitive to be called"),
            primitiveType: PRIMITIVE_TYPES.describe("The type of the primitive to be called"),
            prompt: z9.string().describe("The json string or text value to be sent to the primitive"),
            selectionReason: z9.string().describe("The reason you picked the primitive")
          })
        },
        requestContext,
        maxSteps: 1,
  ...
  const toolStep = createStep({
    ...
    resumeSchema: z9.object({
      approved: z9.boolean().describe("Controls if the tool call is approved or not, should be true when approved and false when declined")
    }),
    execute: async ({ inputData, getInitData, writer, resumeData, mastra, suspend }) => {
      ...
      if (toolRequiresApproval) {
        ...
        if (!resumeData) {
          const approvalSchema = z9.object({
            approved: z9.boolean().describe(
              "Controls if the tool call is approved or not, should be true when approved and false when declined"
            )
          });
  ...
  const networkWorkflow = createWorkflow({
    ...
    outputSchema: z9.object({
      ...
      result: z9.string(),
      isComplete: z9.boolean().optional(),
      completionReason: z9.string().optional(),
      iteration: z9.number(),
      threadId: z9.string().optional(),
      threadResourceId: z9.string().optional(),
      isOneOff: z9.boolean()
...
async function networkLoop({
  ...
  const validationStep = createStep({
    ...
    outputSchema: z9.object({
      ...
      prompt: z9.string(),
      result: z9.string(),
      structuredObject: z9.any().optional(),
      isComplete: z9.boolean().optional(),
      completionReason: z9.string().optional(),
      iteration: z9.number(),
      validationPassed: z9.boolean().optional(),
      validationFeedback: z9.string().optional()
    }),
    execute: async ({ inputData, writer }) => {
          ...
          execute: async (inputData, context) => {
            ...
            try {
              ...
              if (result?.status === "success") {
                ...
                const normalizedResumeSchema = resumeSchema ? toStandardSchema(resumeSchema) : void 0;
  ...
  async moderateContent(content, isStream = false, observabilityContext) {
    ...
    try {
      ...
      const schema = z9.object({
        category_scores: z9.array(
          z9.object({
            category: z9.enum(this.categories).describe("The moderation category being evaluated"),
            score: z9.number().min(0).max(1).describe("Confidence score between 0 and 1 indicating how strongly the content matches this category")
          })
        ).describe("Array of flagged categories with their confidence scores").nullable(),
        reason: z9.string().describe("Brief explanation of why content was flagged").nullable()
      });
      let result;
      if (isSupportedLanguageModel(model)) {
...
var PromptInjectionDetector = class _PromptInjectionDetector {
  ...
  async detectPromptInjection(content, observabilityContext) {
    ...
    try {
      ...
      const baseSchema = z9.object({
        categories: z9.array(
          z9.object({
            type: z9.enum(this.detectionTypes).describe("The type of attack detected from the list of detection types"),
            score: z9.number().min(0).max(1).describe("Confidence level between 0 and 1 indicating how certain the detection is")
          })
        ).nullable(),
        reason: z9.string().describe("The reason for the detection").nullable()
      });
      let schema = baseSchema;
      if (this.strategy === "rewrite") {
        schema = baseSchema.extend({
          rewritten_content: z9.string().describe("The rewritten content that neutralizes the attack while preserving any legitimate user intent").nullable()
        });
      }
      let result;
      if (isSupportedLanguageModel(model)) {
...
var PIIDetector = class _PIIDetector {
  ...
  async detectPII(content, observabilityContext) {
    ...
    try {
      const model = await this.detectionAgent.getModel();
      const baseDetectionSchema = z9.object({
        type: z9.string().describe("Type of PII detected"),
        value: z9.string().describe("The actual PII value found"),
        confidence: z9.number().min(0).max(1).describe("Confidence of this detection"),
        start: z9.number().describe("Start position in the text"),
        end: z9.number().describe("End position in the text")
      });
      const detectionSchema = this.strategy === "redact" ? baseDetectionSchema.extend({
        redacted_value: z9.string().describe("Redacted version of the value").nullable()
      }) : baseDetectionSchema;
      const baseSchema = z9.object({
        categories: z9.array(
          z9.object({
            type: z9.enum(this.detectionTypes).describe("The type of PII detected from the list of detection types"),
            score: z9.number().min(0).max(1).describe("Confidence level between 0 and 1 indicating how certain the detection is")
          })
        ).describe("Array of detected PII types with their confidence scores").nullable(),
        detections: z9.array(detectionSchema).describe("Array of specific PII detections with locations").nullable()
      });
      const schema = this.strategy === "redact" ? baseSchema.extend({
        redacted_content: z9.string().describe("The content with all PII redacted according to the redaction method").nullable()
      }) : baseSchema;
      let result;
      if (isSupportedLanguageModel(model)) {
        const response = await this.detectionAgent.generate(prompt, {
...
var LanguageDetector = class _LanguageDetector {
  ...
  async detectLanguage(content, observabilityContext) {
    ...
    try {
      const model = await this.detectionAgent.getModel();
      const baseSchema = z9.object({
        iso_code: z9.string().describe("ISO language code").nullable(),
        confidence: z9.number().min(0).max(1).describe("Detection confidence").nullable()
      });
      const schema = this.strategy === "translate" ? baseSchema.extend({
        translated_text: z9.string().describe("Translated text").nullable()
      }) : baseSchema;
      let result;
      if (isSupportedLanguageModel(model)) {
        const response = await this.detectionAgent.generate(prompt, {
  ...
  listTools(_config) {
    ...
    return {
      updateWorkingMemory: createTool({
        ...
        inputSchema: z9.object({ memory: z9.string() }),
        execute: async (inputData, context) => {
  ...
  async getWorkingMemoryTemplate({
    ...
    if (workingMemoryConfig.schema) {
      try {
        ...
        let convertedSchema;
        convertedSchema = standardSchemaToJSONSchema(toStandardSchema(schema));
...
//# sourceMappingURL=chunk-6OYGEMV6.js.map
//# sourceMappingURL=chunk-6OYGEMV6.js.map