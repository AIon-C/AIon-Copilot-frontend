...
function isToolLoopAgentLike(obj) {
  if (!obj) return false;
  if (obj instanceof chunk3UVEHZM4_cjs.ToolLoopAgent) return true;
  ...
  async handlePrepareStep(args, currentResult) {
    if (this.settings.prepareStep) {
      ...
      if (currentResult.model) {
        const resolvedModel = await chunk2AWGYDG4_cjs.resolveModelConfig(currentResult.model);
        if (!chunkF5254W6E_cjs.isSupportedLanguageModel(resolvedModel)) {
...
function toolLoopAgentToMastraAgent(agent, options) {
  ...
  const id = agentConfig.id || options?.fallbackName || `tool-loop-agent-${chunkSPDQUG6O_cjs.generateId()}`;
  return new chunkF5254W6E_cjs.Agent({
    ...agentConfig,
    id,
...
//# sourceMappingURL=chunk-7ZYGR5WJ.cjs.map
//# sourceMappingURL=chunk-7ZYGR5WJ.cjs.map