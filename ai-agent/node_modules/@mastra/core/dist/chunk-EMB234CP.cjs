...
var chunkF5254W6E_cjs = require('./chunk-F5254W6E.cjs');
...
var StepExecutor = class extends chunkRO47SMI7_cjs.MastraBase {
  ...
  async execute(params) {
    ...
    const { inputData, validationError } = await chunkF5254W6E_cjs.validateStepInput({
    ...
    try {
      ...
      const stepOutput = await step.execute(
        chunkF5254W6E_cjs.createDeprecationProxy(
          {
            ...
            getStepResult: chunkF5254W6E_cjs.getStepResult.bind(this, stepResults),
            suspend: async (suspendPayload, suspendOptions) => {
              const { suspendData, validationError: validationError2 } = await chunkF5254W6E_cjs.validateStepSuspendData({
            ...
            ...chunkW26XAFT3_cjs.createObservabilityContext(params.tracingContext)
          },
          {
            paramName: "runCount",
            deprecationMessage: chunkF5254W6E_cjs.runCountDeprecationMessage,
            logger: this.logger
          }
        )
      );
      ...
      return {
        ...
        tripwire: error instanceof chunkF5254W6E_cjs.TripWire ? {
      ...
      chunkF5254W6E_cjs.createDeprecationProxy(
        {
          ...
          abortSignal: abortController?.signal,
          // TODO
          ...chunkW26XAFT3_cjs.createObservabilityContext(),
          iterationCount
        },
        {
          paramName: "runCount",
...
function createTripWireFromChunk(chunk) {
  const { payload } = chunk;
  return new chunkF5254W6E_cjs.TripWire(
  ...
  if (resumeData !== void 0 && forEachIndex === void 0 && currentResult?.output?.length > 0) {
    ...
    if (suspendedIndices.length > 0) {
      ...
      for (const suspIdx of indicesToResume) {
        updatedOutput[suspIdx] = chunkSHN25XWN_cjs.createPendingMarker();
      }
  ...
  async processWorkflowStepRun({
    ...
    if (step.step instanceof EventedWorkflow || step.step.component === "WORKFLOW") {
      if (resumeSteps?.length === 1 && resumeSteps[0] === step.step.id) {
        ...
        const timeTravelParams = chunkF5254W6E_cjs.createTimeTravelExecutionParams({
    ...
    const { resumeData: timeTravelResumeData, validationError: timeTravelResumeValidationError } = await chunkF5254W6E_cjs.validateStepResumeData({
...
function cloneWorkflow(workflow, opts) {
  const wf = new chunkF5254W6E_cjs.Workflow({
    id: opts.id,
...
function isAgent(input) {
  return input instanceof chunkF5254W6E_cjs.Agent;
}
function isToolStep(input) {
  return input instanceof chunkE7PW4ZAB_cjs.Tool;
}
function isStepParams(input) {
  return input !== null && typeof input === "object" && "id" in input && "execute" in input && !(input instanceof chunkF5254W6E_cjs.Agent) && !(input instanceof chunkE7PW4ZAB_cjs.Tool);
}
function isProcessor(obj) {
  return obj !== null && typeof obj === "object" && "id" in obj && typeof obj.id === "string" && !(obj instanceof chunkF5254W6E_cjs.Agent) && !(obj instanceof chunkE7PW4ZAB_cjs.Tool) && (typeof obj.processInput === "function" || typeof obj.processInputStep === "function" || typeof obj.processOutputStream === "function" || typeof obj.processOutputResult === "function" || typeof obj.processOutputStep === "function");
}
...
async function processAgentStream(params) {
  ...
  for await (const chunk of fullStream) {
    ...
    if (streamFormat !== "legacy") {
      await chunkF5254W6E_cjs.forwardAgentStreamChunk({ writer, chunk });
    }
  }
  try {
    await pubsub.publish(`workflow.events.v2.${runId}`, {
...
function createStepFromAgent(params, agentOrToolOptions) {
  ...
  return {
    ...
    execute: async ({
      ...
      const observabilityContext = chunkW26XAFT3_cjs.resolveObservabilityContext(obsFields);
      const logger = mastra?.getLogger();
      const toolData = {
        name: params.name,
        args: inputData
      ...
      const isV2Model = chunkF5254W6E_cjs.isSupportedLanguageModel(modelInfo);
...
function createStepFromProcessor(processor) {
  ...
  return {
    ...
    inputSchema: chunkVWJ7H3MJ_cjs.toStandardSchema(chunkF5254W6E_cjs.ProcessorStepSchema),
    outputSchema: chunkVWJ7H3MJ_cjs.toStandardSchema(chunkF5254W6E_cjs.ProcessorStepOutputSchema),
    execute: async ({ inputData, requestContext, ...obsFields }) => {
      ...
      const abort = (reason, options) => {
        throw new chunkF5254W6E_cjs.TripWire(reason || `Tripwire triggered by ${processor.id}`, options, processor.id);
      };
      ...
      const executePhaseWithSpan = async (fn) => {
        try {
          ...
          if (error instanceof chunkF5254W6E_cjs.TripWire) {
        ...
        switch (phase) {
          case "input": {
            if (processor.processInput) {
              ...
              if (result instanceof chunkCLKMJINC_cjs.MessageList) {
                ...
                chunkF5254W6E_cjs.ProcessorRunner.applyMessagesToMessageList(
                  result,
          ...
          case "inputStep": {
            if (processor.processInputStep) {
              ...
              const validatedResult = await chunkF5254W6E_cjs.ProcessorRunner.validateAndFormatProcessInputStepResult(result, {
                messageList: passThrough.messageList,
                processor,
                stepNumber: stepNumber ?? 0
              });
...
var EventedWorkflow = class extends chunkF5254W6E_cjs.Workflow {
  constructor(params) {
    super(params);
...
var EventedRun = class extends chunkF5254W6E_cjs.Run {
  constructor(params) {
    super(params);
  ...
  stream({
    ...
    this.streamOutput = new chunkF5254W6E_cjs.WorkflowRunOutput({
      runId: this.runId,
      workflowId: this.workflowId,
      stream
    });
...
var EventedExecutionEngine = class extends chunkF5254W6E_cjs.ExecutionEngine {
  eventProcessor;
  constructor({
    mastra,
    eventProcessor,
  ...
  async execute(params) {
    ...
    const finishCb = async (event, ack) => {
      ...
      if (["workflow.end", "workflow.fail", "workflow.suspend"].includes(event.type)) {
        ...
        if (event.type === "workflow.fail" && event.data.stepResults) {
          event.data.stepResults = chunkF5254W6E_cjs.hydrateSerializedStepErrors(event.data.stepResults);
        }
    ...
    for (const [stepId, stepResult] of Object.entries(stepResultsWithoutTopLevelState)) {
      cleanStepResults[stepId] = chunkF5254W6E_cjs.cleanStepResult(stepResult);
    }
    let callbackArg;
...
//# sourceMappingURL=chunk-EMB234CP.cjs.map
//# sourceMappingURL=chunk-EMB234CP.cjs.map